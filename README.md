...
- **apps/tyler-site** - Marketing website with Puck visual page builder (Next.js + Tailwind)
...
The backend uses Prisma with PostgreSQL to manage the database schema and operations.

### Prerequisites

1. **PostgreSQL Database**: You need a PostgreSQL database running. You can:
   - Install PostgreSQL locally
   - Use a cloud service like AWS RDS, Heroku Postgres, or Supabase
   - Run PostgreSQL with Docker: `docker run --name postgres -e POSTGRES_PASSWORD=password -p 5432:5432 -d postgres`
...
1. **Configure Database Connection**:
   ...
   ```env
   DATABASE_URL="postgresql://username:password@localhost:5432/tyler_platform?schema=public"
   ```
...
3. **Run Database Migrations**:
   ...
   - Users, Shops, Barbers, Services, Customers
   - Appointments with status tracking
   - Customer CRM (Notes, Flags, Tags)
   - Barber scheduling (Working Hours, Time Off)
   - Payment records and no-show policies
   - Shop theming and custom pages
...
- **Multi-tenant Architecture**: Each shop is isolated with `shopId` references
- **Appointment Management**: Full booking lifecycle with Square integration
- **Customer CRM**: Notes, flags, and tags for customer management
- **Staff Scheduling**: Working hours and time-off management
- **Payment Processing**: Integration with Square payments
- **Shop Customization**: Themes and custom page content
- **No-Show Policies**: Automated fee handling
